...
## 关于 LLM 非确定性：为什么这不是 bug

如果你对比 `outputs/` 里的静态交付物和前端实时导入 SRT 后跑出来的结果，你会发现它们不完全一样——Agent 1 对同一份 SRT 的两次运行，在观点选择、措辞排序和置信度打分上会有差异。

这不是缺陷，而是 LLM 本身的特性（非确定性采样）。更重要的是，**这证明了编辑判断不可替代**：如果同一个工具对同一份素材输出两次不同结果，那么"最终采用哪个观点、以什么顺序呈现、哪些证据需要核实"这些判断就必须由人来完成。AI 提供的是概率性的建议，不是确定性的答案。

如果你在面试中想验证这一点：导入 SRT → 记录第一组观点 → 点"重新分析" → 对比差异。两个版本之间的 delta 本身就是方案设计中"人机协作"为什么成立的论据。
