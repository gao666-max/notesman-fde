# 产出 2：方案设计 — AI 增强内容复盘工具

## 一、整体架构

```
                          ┌──────────────────────┐
                          │   编辑交互前端        │
                          │ ┌──────────────────┐ │
                          │ │ 四象限全局概览    │ │
                          │ │ (置信度×热点)    │ │
                          │ └──────────────────┘ │
                          │ ┌──────┬───────────┐ │
                          │ │卡片库 │ 编辑工作区 │ │
                          │ │(抽屉) │ (拖拽拼接) │ │
                          │ └──────┴───────────┘ │
                          └──────────────────────┘
                                    ▲
                                    │ viewpoints.json + draft
                                    │
┌───────────────────────────────────────────────────────┐
│                    AI 处理管道                         │
│                                                       │
│  SRT ──→ Agent 1 ──→ Agent 2 ──→ [编辑选择角度]       │
│          观点提取      复用判断        │                │
│          (Prompt)    (Prompt+检索)     ▼                │
│                                    Agent 3 ──→ Agent 4 │
│                                    内容生成     事实核查 │
│                                   (Prompt)   (Prompt+RAG)│
│                                                       │
│  输入层: SRT逐字稿 + 8篇历史笔记 + 3篇样稿 + 外部知识库  │
│  输出层: 结构化JSON → 知识图谱前端 → 可编辑初稿           │
└───────────────────────────────────────────────────────┘
```

### 为什么叫"多 Agent"但不用 Agent 框架

每个模块有独立的**角色定义、Prompt、输入输出契约、质量标准和故障处理策略**。它们像一个编辑团队的四个专业角色——观点编辑、资料编辑、撰稿人、事实核查员——各自独立工作、通过结构化文件交接。这个设计不依赖 LangGraph 或 CrewAI，而是用**多步 Prompt 链 + 中间文件协议**实现相同的架构效果。

调研中 lumentis（1.7k⭐）、buildnext-oss、onepod-Skill 均采用此模式。调研中 AWS、Contently、Markup AI 的多 Agent 架构理念——"每个 Agent 只做一件事、独立可替换、通过结构化数据交接"——在本方案中被完整保留。

---

## 二、Agent 1：观点提取

### 角色定位
相当于一个资深编辑通读整篇逐字稿，用三色荧光笔标出"高维思想""当下解答""信息差"三类内容，并在每个标注旁写下证据位置、置信度、热点匹配和风险提示。

### 输入
- `interview_transcript.srt`：97 段带说话人和时间戳的逐字稿
- `content_brief.md`：目标读者、发布渠道、时效要求
- 当前热点话题列表（预定义，非实时爬取）

### AI 介入方式：Prompt
任务明确、输入确定、无外部检索需求——**Prompt 是最合适的工具**。

核心 Prompt 策略（详见 `agent1_prompt.md`）：
- **角色设定**："你是笔记侠资深商业内容编辑，从长篇访谈中提取有发表价值的观点"
- **三维度分类**：high_order_insight（高维思想）、current_answer（当下解答）、info_gap（信息差）
- **证据溯源强制约束**：每个观点至少附带一条逐字稿原文引用+时间戳
- **置信度三级标注**：high（可逐句回溯）、medium（AI 做了语义延伸）、low（AI 推测了未明说含义）
- **热点匹配**：与预定义热点列表做语义关联判断
- **编辑标注**：fact_check_needed/sensitive_content/needs_human_judgment + 具体原因

### 输出
`viewpoints.json`：10-15 个结构化观点节点，JSON 格式供前端渲染知识图谱。

数据结构（详见已生成的原型输出）：
```json
{
  "viewpoint_id": "vp_01",
  "title": "观点小标题（≤12字，有判断力）",
  "category": "high_order_insight | current_answer | info_gap",
  "speaker": "说话人",
  "timestamp_start": "00:00:00",
  "evidence_quotes": [{ "text": "原文", "speaker": "", "timestamp": "" }],
  "confidence": "high | medium | low",
  "keywords": [],
  "hotspot_match": { "matched": true/false, "topic": "", "score": 0.0 },
  "editorial_flags": { "fact_check_needed": false, "sensitive_content": false, "needs_human_judgment": false },
  "summary_short": "一句话摘要（≤40字）"
}
```

### 编辑做什么
- 审核 Agent 1 提出的 10-15 个观点——删掉不认同的、补充 AI 漏掉的
- 调整分类（比如把 AI 分到 info_gap 的观点移到 high_order_insight）
- 确认 `editorial_flags` 的标注是否合理

### 质量 Rubric

| 维度 | 好 | 差 |
|------|----|----|
| **观点密度** | 每个节点是一个独立的、有判断力的观点，不是"AI很重要"这种泛泛而谈 | 输出的是逐字稿摘要而非观点提炼 |
| **证据覆盖** | 每个节点至少一条可精准定位的原文引用 | 观点和证据"大概对得上"但时间戳模糊 |
| **分类准确** | 三种维度边界清晰，高层思想不会被当成信息差 | 所有节点都分到同一类 |
| **热点判断** | 匹配的理由具体（"直接回应了XX焦虑"） | 匹配理由为空或泛泛而谈 |

### 数据质量处理
- **转录重复/明显错误**（如"或者AI编程工具或者AI编程工具"）：Agent 1 标注 `transcript_issue: true`，可推断正确内容写入 `inferred_text`，但**不覆盖**原文 `evidence_quotes` 中的原始转录
- **嘉宾表达跳跃**：不做强制合并。让一个观点节点只承载一段逻辑完整的对话。跳跃的上下文拆成独立节点
- **SRT 开头截断**（第1段主持人被截断）：如不影响核心观点提取，忽略；如影响，标注在 `editorial_checks` 中

---

## 三、Agent 2：历史复用判断

### 角色定位
相当于一个资料编辑——手里有 8 篇过往笔记的索引，每新来一个选题，ta 快速判断：哪些过往内容可以用、哪些不能用、哪些是陷阱。

### 输入
- `viewpoints.json`（Agent 1 输出）
- `historical_notes.jsonl`（8 篇脱敏历史笔记）

### AI 介入方式：Prompt + 结构化检索

8 篇历史笔记的总长度可控（总字符数约 400K），不需要上向量数据库。**直接将 8 篇笔记的标题、日期、主题、适用读者、可引用范围和前 500 字摘要塞入 Prompt 上下文**，让 LLM 对每篇做多维判断。

为什么不是 RAG：
- 8 篇的数量级不需要向量检索——全量塞入上下文即可
- 复用判断的核心不是"语义相似度"，而是**多维判断**：日期是否过时？读者是否匹配？是否同一场？引用范围是否允许？——这些需要 LLM 的推理能力，不是向量检索的相似度排序

### 判断矩阵（LLM 对每篇笔记回答以下问题）

| 判断维度 | 问题 | 选项 |
|---------|------|------|
| 内容唯一性 | 这篇笔记和当前逐字稿是同一场访谈吗？ | 是→不复用 / 否→继续判断 |
| 时效性 | 这篇笔记的日期距今多久？话题是否还有效？ | <6个月→可能可用 / >1年→仅背景参考 |
| 读者匹配 | 这篇笔记的适用读者和当前目标读者重叠吗？ | 重叠→可用 / 不重叠→标注差异 |
| 引用范围 | 这篇笔记的可引用范围允许当前用途吗？ | 允许→可用 / 建议核实→标注 / 禁止→不复用 |
| 观点增强 | 这篇笔记能补充或强化 Agent 1 提取的哪些观点？ | 列出具体 vp_id |

### 输出
`reuse_suggestions.md`：每篇笔记一个判定结论（不复用/有限复用/明确复用）+ 具体理由。

### Agent 2 的特殊设计：同一场陷阱检测
题目材料中的 note_02 与当前逐字稿是**同一场访谈的历史版本**。Agent 2 必须在第一步就识别这个关系：
1. 先做"内容指纹"比对——note_02 标题"关于AI、能动性与未来的对话"与访谈主题高度吻合
2. 标注为 ❌ 不复用
3. 额外建议：如编辑想对照新旧版本，note_02 可作为"差异化基准"——新稿必须在观点选择和组织方式上与旧版有明显区别

### 编辑做什么
- 审核 Agent 2 的复用判断，特别关注"有限复用"——编辑最终决定用不用
- 如果编辑知道 Agent 2 不知道的信息（比如某篇笔记的版权状态），直接覆盖 AI 判断

### 质量 Rubric

| 维度 | 好 | 差 |
|------|----|----|
| **陷阱检测** | 准确识别 note_02 为同一场并标记不复用 | 把 note_02 当"主题相关"推荐复用 |
| **时效判断** | 2023 年的内容标注过时或仅背景参考 | 不对日期做任何判断 |
| **可用性判定** | "可用于XX"有具体理由 | "可以用"没有理由 |

---

## 四、Agent 3：内容生成

### 角色定位
相当于一个熟悉笔记侠风格的撰稿人——拿到编辑选定的角度和观点素材后，按出版标准写出初稿。ta 不决定"写什么"（这是 Agent 1 和编辑的事），ta 决定"怎么写"。

### 前置：编辑选择角度
在 Agent 3 启动之前，编辑从 Agent 1 输出的 3 个切入角度中选择 1 个（或自定义）。这是一个**强制性人工关卡**——Agent 3 不会自动选择角度。

### 输入
- 编辑选定的切入角度（来自 `angles.md` 或自定义）
- 选定角度对应的观点节点列表（从 `viewpoints.json` 中筛选）
- 3 篇样稿的风格特征（由预处理步骤提取，见下文"风格预处理"）
- `content_brief.md`：目标读者、渠道

### AI 介入方式：Prompt（含样稿风格约束）

为什么不是 Agent：
- 任务高度结构化——给定观点+大纲+风格→写出分段草稿
- 不需要自主决策——Agent 3 不决定"写什么"，只决定"怎么写"

Prompt 核心策略：
- **风格约束**：从样稿中归纳的笔记侠风格特征作为 Prompt 的一部分
- **证据锚定**：每个段落必须能回到 `viewpoints.json` 中的 `evidence_quotes`
- **长度控制**：分段生成，每段 150-200 字，总长 500-800 字
- **可编辑性**：输出是 Markdown，保留结构标记，编辑可以直接修改

### 样稿风格预处理（手动+AI辅助）
3 篇 published_samples 的风格特征（已完成归纳）：
- **标题模式**：人名/公司名 + 冒号 + 核心观点句
- **开头结构**：内容来源 + 责编 + 第X篇深度好文 + "笔记君说"推荐语
- **正文组织**：背景导入 → 大标题分段 → 每段有案例/数据 → 结尾升华
- **引用方式**：不直接标时间戳，但每段可回溯到原始演讲/访谈
- **字数**：深度文章 5000-10000 字；本原型目标 500-800 字（演示用）

### 输出
`draft_v0_5.md`：500-800 字可编辑初稿，含时间戳标注、待确认标记。

### 编辑做什么
- **结构层**：调整段落顺序，拆分或合并段落
- **表达层**：修改具体措辞，添加自己的判断和金句
- **事实层**：确认或修正 AI 标注的待核实项

### 质量 Rubric

| 维度 | 好 | 差 |
|------|----|----|
| **风格一致** | 读起来像笔记侠文章，不是 ChatGPT 文章 | 有明显的"AI 味"——每段开头都是"在当今时代..." |
| **证据保真** | 关键论述能回到观点节点→回到逐字稿 | 文章流畅但找不到原始出处 |
| **结构可用** | 编辑拿到可以在此基础上改，不需要推翻重写 | 编辑需要重新搭结构 |
| **编辑判断保留** | AI 标注了哪些地方它不确定 | 所有地方看起来"一样确定" |

---

## 五、Agent 4：事实核查

### 角色定位
相当于一个"吹毛求疵"的事实核查编辑——拿到初稿后，逐句往回比对原始证据。AI 做的论断 AI 自己来查，嘉宾说的外部数据 AI 去外部知识库验证。

### 输入
- `draft_v0_5.md`（Agent 3 输出）
- `interview_transcript.srt`（原始逐字稿——内检基准）
- `viewpoints.json`（观点节点及其证据时间戳）

### AI 介入方式：Prompt + RAG（双通道）

Agent 4 分两个通道：

**通道 1：内检（Prompt）**——草稿中的每个论断 vs 原始 SRT
- 逐句比对：这句话在逐字稿中有没有对应的原文？
- 语义一致性：AI 有没有把"可能""也许"改成"一定""必然"？
- 说话人归属：AI 有没有把嘉宾A 说的话写成嘉宾B？

**通道 2：外检（RAG）**——草稿中标注 `fact_check_needed: true` 的论断 vs 外部可信知识库
- 检索源：学术论文摘要（arXiv/Semantic Scholar）、权威媒体报道、维基百科
- 流程：论断→向量化→检索→返回 top-k 相关文档→LLM 判断"支撑/部分支撑/无支撑/矛盾"
- 外检只在编辑开启时才执行——因为检索结果需要编辑解读，不能自动采纳

### RAG 外检的技术路径

```
标注 fact_check_needed 的句子
      │
      ▼
句子向量化（text-embedding-3-small 或 bge-large-zh）
      │
      ▼
检索（在预建的可信知识库索引中做余弦相似度搜索，Top-K=5）
      │
      ▼
LLM 判断：对每篇返回的文档，判断其与论断的关系
  - "支撑"：文档提供了支持该论断的证据
  - "部分支撑"：方向一致但细节有差异
  - "无支撑"：找不到相关证据
  - "矛盾"：找到了与该论断相反的证据
      │
      ▼
输出：每句待核实论断 + 检索结果 + 判断 + 来源链接
```

### 输出
`editorial_checks.md`：分为六个部分——
1. 事实核查项（需核实外部数据+检索结果）
2. 转录含混项（SRT 中语义不完整的段落）
3. 事实不确定项（嘉宾表述模糊或可能不准确）
4. 观点冲突或需编辑判断项（可能引发争议的表述）
5. 数据缺失项（无视频、样稿读取问题等）
6. 历史素材复用警告（是否存在同一场/过时/主题不匹配）

### 编辑做什么
- 逐条审核 Agent 4 的核查结果
- 决定：修改原文？标注"据嘉宾引用的研究"？删除？
- 外检结果不能自动采纳——编辑必须打开来源链接确认

### 外检结果呈现（在前端）

每句待核实论断在前端显示为**可点击的黄色高亮**。点击弹出侧栏：
- 论断原文
- RAG 检索结果（支撑/部分支撑/无支撑/矛盾）+ 置信度
- 建议编辑动作（修改表述/加注脚/删除/保留但标注）
- "打开来源"链接

### 质量 Rubric

| 维度 | 好 | 差 |
|------|----|----|
| **内检覆盖率** | 初稿中每个带时间戳的论述都被比对过 | 只检查了部分段落 |
| **误报控制** | flag 的都是真问题，没有"狼来了" | 大量 flag 是正确但 AI 误判的 |
| **外检溯源** | 检索结果有可点击的来源链接 | 给出了判断但不知道依据是什么 |
| **编辑可操作性** | 每个标注有"建议动作"，编辑不用自己猜怎么改 | 只说"可能有问题"但不给建议 |

---

## 六、编辑交互前端（知识图谱可视化）

### 设计理念
不是仪表盘，不是报表。是**编辑的决策辅助面板**——AI 把逐字稿预加工成可拖拽的知识卡片，编辑做的是选择、排序、拼接，而不是从零开始写。

### 界面布局

```
┌──────────────────────────────────────────────────────────────┐
│  顶部工具栏：[导入SRT] [导入历史笔记] [重新分析] [导出初稿]    │
├──────────────────────────────────────────────────────────────┤
│  四象限全局概览（可选折叠）                                    │
│  纵轴：置信度（高→低）  横轴：热点相关性（低→高）               │
│  🟢高置信+高热点（优先写）  🟡低置信+高热点（需核实）            │
│  🟢高置信+低热点（信息宝藏）  🔴低置信+低热点（可跳过）          │
├─────────────────────┬────────────────────────────────────────┤
│ 左侧：抽屉卡片库      │ 右侧：编辑工作区                        │
│ （按维度分抽屉）      │                                       │
│                     │  ┌── 引言 ──────────────────────┐     │
│ ▼ 高维思想 (5)      │  │ [已拖入] vp_01 企业家=能动性   │     │
│  ┌──────────────┐  │  │ 嘉宾A 00:00:04 🟢高置信       │     │
│  │ vp_01        │  │  └──────────────────────────────┘     │
│  │ 企业家=能动性 │  │                                       │
│  │ 嘉宾A 00:00  │  │  ┌── 正文一：差距在扩大 ────────┐     │
│  │ 🟢 高置信    │  │  │ [已拖入] vp_08 杠铃效应       │     │
│  │ 🔥 AI职业    │  │  │ [已拖入] vp_12 不适应者收入降  │     │
│  └──────────────┘  │  │ 🔴 此卡片标注"待核实数据"     │     │
│  ┌──────────────┐  │  └──────────────────────────────┘     │
│  │ vp_02        │  │                                       │
│  │ 工业革命≠自动│  │  ┌── 结尾：怎么办 ────────────┐       │
│  │ 嘉宾A 00:10  │  │  │ [空] 从左侧拖一个观点进来     │     │
│  │ 🟢 高置信    │  │  └──────────────────────────────┘     │
│  │ 🔥 AI失业    │  │                                       │
│  └──────────────┘  │       [生成全文草稿] 按钮               │
│                     │       [Agent 4 事实核查] 按钮           │
│ ▼ 当下解答 (4)      │                                       │
│ ▼ 信息差 (4)        │                                       │
│                     │                                       │
│ 🔴 仅低置信 (2)    │                                       │
└─────────────────────┴────────────────────────────────────────┘
```

### 交互行为

| 操作 | 效果 |
|------|------|
| 点击左侧卡片 | 右侧弹出详情：证据原文、完整时间戳、编辑标注、热点匹配理由 |
| 拖拽卡片到右侧工作区 | 卡片被添加到对应区域（引言/各段/结尾） |
| 在工作区拖动卡片排序 | 调整段落内观点的出场顺序 |
| 右击工作区卡片→"展开" | 用 Agent 3 的 Prompt 对该观点做 150 字扩写 |
| 右击工作区卡片→"移除" | 从当前大纲中移除（卡片回到左侧库） |
| 点击 🟡/🔴 置信度标签 | 弹出 Agent 4 的核查详情和编辑建议 |
| 点击 [生成全文草稿] | 调用 Agent 3，把右侧工作区的结构+卡片扩写成完整文章 |
| 点击 [Agent 4 核查] | 调用 Agent 4，对当前草稿执行内检+（可选）外检 |

### 卡片右下角信息
每张卡片右下角显示：`说话人 | 时间戳 | 置信度标签颜色`。编辑不点开卡片也能快速定位证据来源。

### 前端技术选型建议
- **框架**：React + TypeScript（交互状态管理成熟）
- **拖拽**：dnd-kit 或 react-beautiful-dnd
- **四象限图**：ECharts 散点图（中文文档好，上手快）
- **卡片渲染**：纯 CSS Grid/Flexbox
- **数据驱动**：所有卡片数据来自 `viewpoints.json`

---

## 七、数据缺失、噪声与冲突信号的处理策略

| 信号类型 | 检测方式 | 处理方式 | 责任归属 |
|---------|---------|---------|---------|
| SRT 转录重复/乱码 | Agent 1 检测疑似重复或格式异常的段落 | 标注 `transcript_issue: true`；推断正确内容写入 `inferred_text`；**不覆盖**原始转录 | 编辑决定是否采纳推断 |
| 嘉宾表述跳跃/断裂 | Agent 1 检测到上下文逻辑不连贯 | 不强制合并——拆成独立节点，降低置信度为 medium | 编辑在写作阶段决定是否弥合 |
| 嘉宾表述含混 | Agent 1 检测到语义不完整 | 降低置信度为 low，标注 `needs_human_judgment: true` | AI 不填补嘉宾没说清楚的内容 |
| 两个嘉宾观点冲突 | Agent 2 检测同一话题下两个嘉宾的判断不一致 | 保留双方观点，标注"观点分歧：嘉宾A认为X，嘉宾B认为Y"。AI 不做仲裁 | 编辑决定：都呈现/只用一个/标注为"不同视角" |
| 历史笔记与当前素材重复 | Agent 2 识别 note_02 等同一场内容 | 标注为 ❌ 不复用 | 编辑确认 |
| 历史笔记过时 | Agent 2 检查日期（>1 年）和可引用范围 | 标注"仅背景参考" | 编辑决定是否保留 |
| 缺少视频/分镜稿 | — | 在 `editorial_checks.md` 中明示缺失项 | 编辑如有视频可补充判断 |
| 样稿读取失败（sample_03） | 预处理流水线检测 | 不阻塞流程——记录失败、降级为手动读取 | 编辑手动查看并补充风格归纳 |
| 外部数据无法核实 | Agent 4 外检 RAG 未找到匹配 | 标注"未找到独立核实来源"——不假装知道 | 编辑决定是否保留该论断或改为"嘉宾称" |

---

## 八、关键节点的质量 Rubric 汇总

| 节点 | 核心指标 | 测量方式 |
|------|---------|---------|
| Agent 1 观点提取 | 观点密度（每节点独立判断力）、证据覆盖（每条可溯源）、分类准确 | LLM-as-judge：另一个 LLM 独立检查每个节点是否能回原文、分类是否合理 |
| Agent 2 复用判断 | 陷阱检测率（note_02 是否被正确识别）、时效判断准确率 | 编辑人工审核 |
| 编辑角度选择 | —（纯人工步骤，不设自动化指标） | — |
| Agent 3 内容生成 | 风格一致性、证据保真、结构可用性 | 编辑主观评分 + LLM 比对草稿 vs 观点节点的语义偏移量 |
| Agent 4 事实核查 | 内检覆盖率、误报率、外检溯源完整性 | 编辑审核标注是否准确 |
| 最终文章 | 观点密度、目标读者匹配度、证据可溯源 | 编辑终审 + 读者反馈 |

---

## 九、人机分工总览

| 环节 | AI 做 | 人做 |
|------|-------|------|
| 逐字稿通读 | ✅ 自动提取观点，输出结构化节点 | ❌ 不再需要通读 97 段 |
| 找"值得写的" | ✅ 三维度分类+置信度+热点匹配，排序推荐 | ✅ 最终决定哪些写、哪些不写 |
| 选角度 | ✅ 生成 3 个备选角度+理由 | ✅ **必须人工选择**——这是不可跳过的决策关卡 |
| 资料复用 | ✅ 8 篇笔记逐一判断 | ✅ 审核判断结果，特别关注有限复用 |
| 写初稿 | ✅ 按选定角度+观点节点生成分段草稿 | ✅ 调整结构、添加个人判断和金句 |
| 事实核查 | ✅ 内检逐句回原文+外检 RAG 搜索 | ✅ 确认每个标注，决定修改/保留/删除 |
| 风格适配 | ✅ 从样稿提取风格特征，应用到生成 | ✅ 微调措辞使风格统一 |
| 最终审稿 | ❌ 不做——AI 不能代编辑拍板 | ✅ **必须由人完成**——这是题目负责人的原话 |

---

## 十、因果链提取器（规划中，原型未实现）

### 为什么需要

口语中一个观点的完整逻辑往往是散落式的——前 5 分钟讲现象，中间穿插例子，后面才给结论。AI 摘要容易把中间推理步骤丢掉，剩下"现象 → 结论"。编辑需要看到完整的"因为 A → 所以 B → 导致 C"链，才能判断这个观点的逻辑是否站得住。

### 设计方案

在 Agent 1 的观点提取阶段增加一个可选输出字段 `causalChain`：

```json
{
  "causalChain": {
    "premise": "AI让写代码的成本降到几乎为零",
    "reasoning": "当非技术人员也能用AI创建应用时，过去依赖技术排期的工作流程就被打破了",
    "conclusion": "产品经理从'指挥家'变成了能独立闭环的角色",
    "evidence": [
      { "segment": "嘉宾A 00:23:06", "text": "现在很多产品经理自己就能写代码..." },
      { "segment": "嘉宾A 00:22:08", "text": "产品经理的这个生命周期，通常可能要花好几个月" }
    ],
    "missingSteps": ["产品经理是否真的不需要设计师？", "这种变化在多大公司范围内成立？"]
  }
}
```

关键设计：
- `missingSteps` 字段是 AI 主动标注"这个因果链还缺什么"——让编辑一眼看到逻辑断点
- 不是每个观点都需要因果链标注，只在观点本身依赖因果推理时标注
- 编辑在前端看到因果链后，可标记"逻辑成立""缺一步""存疑"

### 当前状态

未在原型中实现。原型中 Agent 1 只提取观点+证据标签，不做显式因果标注。Agent 3 生成文章时隐式地做了因果串联，但编辑无法检查推理链是否完整。

---

## 十一、人 × AI 知识四象限（Johari 窗口应用）

将"乔哈里视窗"（Johari Window）框架映射到编辑 × AI 协作场景：

```
            编辑知道                    编辑不知道
         ┌──────────────────┬──────────────────┐
  AI     │                  │                  │
  知道   │  共识区           │  AI 发现区        │
         │  嘉宾明确说了       │  AI 提取了但编辑   │
         │  "企业家=能动性"    │  自己通读时忽略了  │
         │  ✅ Agent 1 绿色高置信│  ⚠️ 弱信号抽屉    │
         │  编辑可直接采用     │  需编辑二次确认    │
         ├──────────────────┼──────────────────┤
  AI     │                  │                  │
  不知道  │  编辑洞察区        │  盲区             │
         │  编辑知道嘉宾去年   │  嘉宾没说但读者    │
         │  说过相反的话       │  真正想问的问题    │
         │  ❌ 当前系统完全未   │  ❌ 无人能覆盖     │
         │  覆盖——编辑的隐     │  但 editorial_    │
         │  性知识无法录入     │  checks.md 的     │
         │  系统               │  "待确认清单"     │
         │                    │  是朝这个方向的   │
         └──────────────────┴──────────────────┘
```

**当前覆盖情况**：
- **共识区**：✅ 做的最好。Agent 1 高置信度观点 + 证据原文，编辑可以快速验证。
- **AI 发现区**：⚠️ 弱信号抽屉是这个象限的入口，但编辑可能忽略它。未来可加"你可能没注意到"的主动推送。
- **编辑洞察区**：❌ 最大缺口。编辑知道"这个话题嘉宾去年说过相反的话""这个数据引用的是过时研究"——这些隐性知识系统完全感知不到。改进方向：在前端加一个"编辑备注"输入框，让编辑可以给任何观点加私有标注。这是让系统"学习"编辑判断的第一步。
- **盲区**：无法消除，但可以缩小。Agent 4 的外检（"这段话在公开报道中能找到独立验证吗"）和 editorial_checks.md 的待确认清单，本质是把盲区变成可追问的问题。

---

## 十二、编辑决策记录（已实现）

### 为什么需要

题目明确指出："不同编辑对'什么值得写、如何组织、哪些表述需核实'的判断不一致——这不是 bug，但需要可见。" 如果同一个素材两个编辑处理结果不同，团队需要知道：谁选了哪些观点？为什么选 A 没选 B？标题改过吗？哪些地方做了事实核查？

### 实现方案（已在前端原型中实现）

每次保存大纲时，自动生成 `changeLog` 记录：

```json
{
  "savedAt": "2026-08-02T01:23:45.000Z",
  "previousState": { "title": "...", "itemCount": 3 },
  "currentState": { "title": "...", "itemCount": 5 },
  "changes": [
    { "action": "添加", "id": "vp_08", "title": "杠铃效应", "speaker": "嘉宾B" },
    { "action": "移除", "id": "vp_11", "title": "能动性=拒绝追求赞美" },
    { "action": "修改标题", "from": "AI时代的...", "to": "AI不会淘汰人..." }
  ]
}
```

当前实现的粒度：记录拖入/拖出/标题修改。未记录（未来可加）：事实核查结果采纳情况、草稿文本修改 diff、编辑添加的私有备注。
